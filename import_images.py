...
def load_images():
    ...
    for file, _ in files_to_load:
        ...
        I = cv2.cvtColor(I, cv2.COLOR_BGR2GRAY).astype(np.float32)
        I = 255 - I
        # pad to square
        # x, y = I.shape
        # max_dim = max(x, y)
        # if max_dim == x:
        #     pad_size = (x - y) // 2
        #     pad_shape = ((0, 0), (pad_size, pad_size))
        # else:
        #     pad_size = (y - x) // 2
        #     pad_shape = ((pad_size, pad_size), (0, 0))
        ...
        # I = np.pad(I, pad_shape, mode="constant")
        # # add border
        # I = np.pad(I, 12, mode="constant")
...
# data_test_na_naszych = cv2.imread("../segmentacja_output/BG_0014_10.jpg")
